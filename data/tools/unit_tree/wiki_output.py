...
def main():
    ...
    base_defines = "NORMAL,ENABLE_ARMAGEDDON_DRAKE,ENABLE_DWARVISH_ARCANISTER," +\
        ...
        "ENABLE_PARAGON,ENABLE_SAURIAN_SPEARTHROWER,DISABLE_GRAND_MARSHAL".split(",")
    ...
    for campaign in campaigns:
        ...
        if define is not None: campaign_defines += define.split(",")
        if ed is not None: campaign_defines += ed.split(",")
    ...
    for campaign, unitslists in list(punits.items()):
        for unitlist in unitslists:
            for race in unitlist.get_all(tag = "race"):
                ...
                if race_id is None:
                    # error_message("Warning: Campaign '%s' defines a race without id." % campaign)
                    continue
                else:
    ...
    for campaign, unitslists in list(punits.items()):
        for unitlist in unitslists:
            for unit in unitlist.get_all(tag = "unit_type"):
                ...
                if unit.id is None:
                    # error_message("Warning: Campaign '%s' defines a unit without id." % campaign)
                    continue
    ...
    def base_val(unit, val, translation = None):
        ...
        for base_unit in unit.get_all(tag = "base_unit"):
            ...
            if base_uid is None or not base_uid in all_units:
                # error_message("Warning: Unit '%s' based on non-existant '%s'." %
                #     (unit.get_text_val("id"), base_uid))
                continue
    ...
    for unit in list(all_units.values()):
        for aid in unit.advances_to:
            if not aid in all_units:
                # error_message("Warning: Unit '%s' advances to non-existant '%s'." %
                #     (unit.get_text_val("id"), aid))
                continue
