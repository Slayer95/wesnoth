...
class ExpandableNode:
    ...
    def replace(self, args_set=NO_ARGS, macro_defs=None):
        ...
        if macro_defs is not None and self.name in macro_defs:
            ...
            if len(self.children) > len(macro_args):
                # Ideally we wouldn't complain and we should just let pertinent tools worry about it,
                # but a wmltools3 bug might cause a perfectly fine macro call to be parsed wrong.
                print("Macro %s expects %d arguments, but has %d arguments." % (str(self), len(macro_args), len(self.children)))
            return result
...
class CrossRefHelper:
    ...
    def _get_valid_macros(input, xrefs):
        ...
        for name in input:
            if name in GlobalWMLMacros:
                ...
                if name.upper() != name:
                    # Parens issue happens because of a bug in wmltools3 @ parse_macroref()
                    print("This error might also be fixed by adding extra parens around any macro arguments.")
    ...
    def deep_replace_macros(input_sentence, variants, xrefs):
        ...
        for defs, sentinel, sentence in CrossRefHelper.get_macro_replaceables(input_sentence, variants):
            ...
            assert(isinstance(defs, Reference) or type(defs) == dict), ("defs is not ReferenceLike: %s" % str(defs))
                ...
                if call_site[1] is not None:
                    for i, called_arg in enumerate(call_site[1]):
                        if i >= len(xref.args):
                            ...
                            # Ideally we wouldn't complain and we should just let pertinent tools worry about it,
                            # but a wmltools3 bug might cause a perfectly fine macro call to be parsed wrong.
                            print("Macro called with more arguments than expected at %s:%d (xref_file_name, call_site[0]). \
This might also be caused by a parser bug.")
                            break
                ...
                if untranslatable_arg is None and call_site[2] is not None:
                    for parameter_name, called_arg in call_site[2].items():
                        if not parameter_name in xref.optional_args:
                            ...
                            # Might as well complain, too.
                            print("Unexpected optional argument \"%s\" at %s:%d." % (parameter_name, xref_file_name, call_site[0]))
                            continue
                        if parameter_name in filter_parameters:
                            if has_quotes(called_arg):
                                untranslatable_arg = called_arg
